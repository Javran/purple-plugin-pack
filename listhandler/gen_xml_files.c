...
static void
lh_generic_import_alias(xmlnode *alist)
{
	xmlnode *buddy = NULL;
...
static void
lh_generic_build_alist_tree(xmlnode *parent)
{
	/* root of tree group contact buddy */
	PurpleBlistNode *root = buddies->root, *g = NULL, *c = NULL, *b = NULL;
	xmlnode *buddy = NULL;
	...
	for(g = root; g; g = g->next) {
		if(PURPLE_BLIST_NODE_IS_GROUP(g)) {
			...
			/* ignore the groups for the alias list tree */
...
static void
lh_generic_export_alist_request_cb(void *user_data, const char *filename)
{
	FILE *export = fopen(filename, "w");

	if(export) {
		int xmlstrlen = 0;
		xmlnode *tree = lh_generic_alist_build_tree();
		char *xmlstring = xmlnode_to_formatted_str(tree, &xmlstrlen);
...
static void
lh_generic_export_alist_cb(void *ignored, PurpleRequestFields *fields)
{
	/* get the source account from the dialog we requested */
	source_account = purple_request_fields_get_account(fields, "generic_source_acct");
	...
	if(buddies)
		purple_request_file(listhandler, _("Save Generic .alist File"), NULL,
				TRUE, G_CALLBACK(lh_generic_export_alist_request_cb), NULL,
				source_account, NULL, NULL, NULL);
	else
		purple_debug_info("listhandler: export", "alist not returned\n");
...
void /* do some work and export the damn blist already */
lh_generic_export_alist_action_cb(PurplePluginAction *action)
{
	PurpleRequestFields *request;
	PurpleRequestFieldGroup *group;
	PurpleRequestField *field;
	...
	purple_request_fields(purple_get_blist(), _("Listhandler - Exporting"),
						_("Choose the account to export from:"), NULL, request,
						_("_Export"), G_CALLBACK(lh_generic_export_alist_cb), _("_Cancel"),
						NULL, NULL, NULL, NULL, NULL);
...
static void
lh_generic_import_alist_target_request_cb(void *ignored, PurpleRequestFields *fields)
{
	/* get the target account */
	target_account = purple_request_fields_get_account(fields, "generic_target_acct");
	...
	/* call separate functions to import the privacy and alist */
	lh_generic_import_privacy(xmlnode_get_child(root, "privacy"));	

	purple_debug_info("listhandler: import", "Parsing Alias List in XML and setting aliases \n");

	lh_generic_import_alias( xmlnode_get_child(root, "alist") );
...
static void
lh_generic_import_alist_target_request(void)
{
	PurpleRequestFields *request;
	PurpleRequestFieldGroup *group;
	PurpleRequestField *field;
	...
	purple_request_fields(purple_get_blist(), _("Listhandler - Importing"),
						_("Choose the account to import to:"), NULL, request,
						_("_Import"),
						G_CALLBACK(lh_generic_import_alist_target_request_cb),
						_("_Cancel"), NULL, NULL, NULL, NULL, NULL);
...
static void
lh_generic_import_alist_request_cb(void *user_data, const char *file)
{
	purple_debug_info("listhandler: import", "Beginning import\n");
	...
	if(file) {
		...
		lh_generic_import_alist_target_request();
	}
}


void
lh_generic_import_alist_action_cb(PurplePluginAction *action)
{
	purple_debug_info("listhandler: import", "Requesting the file.\n");

	purple_request_file(listhandler, _("Choose A Generic Buddy List File To Import"),
			NULL, FALSE, G_CALLBACK(lh_generic_import_alist_request_cb),
			NULL, NULL, NULL, NULL, NULL);
